 task filter_vcf {
	...
	command <<<
		...
		/opt/ccdg/bedtools-2.27.1/bin/bedtools intersect -a ${sample_id}.filtered.vcf -b ${bed} > ${sample_id}.target.filtered.vcf
	...
	output {
		File filtered_vcf = "${sample_id}.target.filtered.vcf"
	}
}